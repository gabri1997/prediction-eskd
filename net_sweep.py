...
def wandb_function(fold):
    wandb.init(
            project="ESKD_NN",
            name=f"ESKD_fold_{fold}",   
            reinit=True)  
...
def train_and_evaluate(df, num_epochs):
    ...
    fold_to_run = wandb.config.fold if 'fold' in wandb.config else 1
...
if __name__ == "__main__":
    ...
    num_epochs = 150
    train_and_evaluate(df, num_epochs)